# Phase 5: 생산 라인

## 목표

`PRODUCING` 주문의 부족분을 생산하고, 생산 완료 시 재고를 반영하며 주문을 `CONFIRMED`로 전환한다.
프로그램이 중간에 종료되어도 재시작 시 생산 완료 여부를 올바르게 판단한다.

## 참고 문서

- [production-line.md](../docs/features/production-line.md)
- [persistence.md](../docs/features/persistence.md)

## 구현 범위

1. **생산 계산**
   - 실 생산량 = `ceil(부족분 / 수율)`
   - 총 생산 시간 = `평균 생산시간 * 실 생산량`
2. **생산 큐**: FIFO 스케줄링으로 대기 중인 생산 작업 관리. Phase 4의 `OrderBook::approve()`는
   재고 부족 시 주문 상태만 `Producing`으로 바꾸고 별도 큐에 등록하지는 않으므로(생산 큐
   도메인 타입이 아직 없었음), 이 Phase에서 큐를 도입하면서 `OrderBook`이 `Producing` 전환
   시 큐에도 등록하도록 연동한다 (참고: `docs_temp/phase4-order-approval.md`).
3. **생산 완료 처리**
   - 재고 Update는 생산이 진행되는 동안이 아니라 완료된 시점에 한 번에 반영
   - 수율은 실 생산량 계산에만 사용되고, 생산 시도한 수량은 모두 재고에 포함 (생산 실패로 인한
     폐기 없음)
   - 완료 시 해당 주문을 `PRODUCING` → `CONFIRMED`로 전환
4. **재시작 시 복구**
   - 저장된 생산 완료 예정 시각을 확인해, 프로그램이 꺼져 있던 동안 이미 완료 시각이 지난
     생산 건은 재시작 시점에 재고 Update와 상태 전환을 반영
5. **생산 현황/대기 목록 표기**: 현재 생산 중인 시료 정보와 대기 큐 목록 출력
6. **생산 큐 영속성**: Phase 1의 `Repository`(`SystemState`)에 생산 큐 항목(주문 참조, 생산
   완료 예정 시각 등)을 저장/불러오기 대상으로 추가한다. Phase 1에서는 생산 큐 도메인 타입이
   아직 없어 이연되었던 부분이다 (참고: `docs_temp/phase1-foundation.md`).

## 완료 기준 (체크리스트)

- [ ] 실 생산량/총 생산 시간 계산식 테스트 작성 및 통과
- [ ] FIFO 큐 동작(투입 순서대로 처리) 테스트 작성 및 통과
- [ ] 생산 완료 시 재고가 정확히 실 생산량만큼 한 번에 증가하는지(중간에 반영되지 않는지) 테스트
  작성 및 통과
- [ ] 프로그램 재시작 시나리오: 생산 완료 예정 시각이 이미 지난 상태로 재시작했을 때 재고/주문
  상태가 즉시 갱신되는지 테스트 작성 및 통과
- [ ] 생산 현황/대기 큐 조회 기능 테스트 작성 및 통과
